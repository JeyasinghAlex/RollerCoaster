public class AdminOperations {
    ...
    private void configureThemePark() {
        if (login()) {
            for (int i = 0; i < ThemePark.games.size(); i++) {
//                System.out.println("Select "+i+1+"for"+ThemePark.games.get(1).getName());
            }
            int selectGameOption = GeneralUtils.getInstance().checkAndReturnValidInteger();
            Game selectedGame = ThemePark.games.get(selectGameOption - 1);
            for (int i = 0; i < ThemePark.games.size(); i++) {
//                System.out.println("Select "+i+1+"to for"+ThemePark.availableOperators.get(1).name);
            }
            int selectedOperatorAction = GeneralUtils.getInstance().checkAndReturnValidInteger();
            Operator selectedOperator = ThemePark.availableOperators.get(selectedOperatorAction - 1);
